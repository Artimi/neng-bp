...
Pro úèely vlastní minimalizace funkce je vhodné pøidat k funkci $v$ penalizaci  pro body, které nele¾í v $\Delta$ podle~\ref{eq:set_mixed_strategies}.
...
\begin{equation}
\textbf{p}_c^{(g+1)} = (1 - c_c) \textbf{p}_c^{(g)} + \sqrt{c_c (2 - c_c) \mu_{eff}} \frac{\textbf{m}^{(g+1)} - \textbf{m}^{(g)}}{\sigma^{(g)}} 
\end{equation}
...
& + c_1 \underbrace{\textbf{p}_c^{(g+1)} \textbf{p}_c^{(g+1)^T}}_\text{rank-one update} \notag \\
...
\item Pokud jsou jednotlivé kroky na sebe kolmé, délka kroku by se mìnit nemìla (Obrázek~\ref{fig:stepsize_cma} uprostøed).
...
\begin{equation}
\textbf{p}_{\sigma}^{(g+1)} = (1 - c_{\sigma}) \textbf{p}_{\sigma}^{(g)} + \sqrt{c_{\sigma}(2-c_{\sigma}) \mu_{eff}} \textbf{C}^{(g)^{-\frac{1}{2}}} \frac{\textbf{m}^{(g+1)} - \textbf{m}^{(g)}}{\sigma^{(g)}}
\end{equation}
kde:
\begin{description}
...
\subsubsection{Kontrola velikosti kroku}
...
\begin{equation}
\mathsf{E} \| \mathcal{N}(\textbf{0},\textbf{I}) \| = \frac{\sqrt{2} \Gamma \left( \frac{N+1}{2} \right)}{\Gamma \left( \frac{N}{2}\right)} \approx \sqrt{N} + \mathcal{O}(\frac{1}{n})
\end{equation}
